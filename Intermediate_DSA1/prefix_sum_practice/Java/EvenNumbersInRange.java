...
public class EvenNumbersInRange {
    public int[] evenNumbers(int[] A, int[][] B) {
        int[] prefixCount = new int[A.length];
        int[] resultArray = new int[B.length];

        prefixCount[0] = (A[0] % 2) == 0 ? 1 : 0;
        ...
        for(int i = 0; i < B.length; i++) {
            ...
            if(L == 0) {
                resultArray[i] = prefixCount[R];
            } else {
                resultArray[i] = prefixCount[R] - prefixCount[L-1];
            }
        }
    ...
    public static void main(String[] args) {
        int[] A = {1, 2, 3, 4, 5};
        int[][] B = {{0, 2}, {2, 4}, {1, 4}};
        EvenNumbersInRange obj = new EvenNumbersInRange();
        int[] result = obj.evenNumbers(A, B);
        System.out.println("Number of even numbers in the given range: " + Arrays.toString(result));
    }
}
