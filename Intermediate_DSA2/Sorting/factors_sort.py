...
# i.e., element having the least number of factors should be the first to be displayed and the number having
...
import functools
def factorSortComparator(val1, val2):
    count1 = getFactors(val1)
    count2 = getFactors(val2) 
    if count1 == count2:
        return val1 - val2
...
def factorSort(A):
    A = sorted(A, key = functools.cmp_to_key(factorSortComparator))
    return A
